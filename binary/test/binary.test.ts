import { SerializedDaemonium-CoreConfig } from "core";
...
const DAEMONIUM_CORE_GLOBAL_DIR = path.join(__dirname, "..", ".daemonium-core");
if (fs.existsSync(DAEMONIUM_CORE_GLOBAL_DIR)) {
  fs.rmSync(DAEMONIUM_CORE_GLOBAL_DIR, { recursive: true, force: true });
}
fs.mkdirSync(DAEMONIUM_CORE_GLOBAL_DIR);
...
describe("Test Suite", () => {
  ...
  beforeAll(async () => {
    ...
    const binaryPath = path.join(binaryDir, `daemonium-core-binary${exe}`);
    const expectedItems = [
      `daemonium-core-binary${exe}`,
      `esbuild${exe}`,
      "index.node",
      "package.json",
    ...
    if (USE_TCP) {
      ...
      try {
        subprocess = spawn(binaryPath, {
          env: { ...process.env, DAEMONIUM_CORE_GLOBAL_DIR },
        });
  ...
  it("should create .daemonium-core directory at the specified location with expected files", async () => {
    expect(fs.existsSync(DAEMONIUM_CORE_GLOBAL_DIR)).toBe(true);
    ...
    const missingFiles = expectedFiles.filter((file) => {
      const filePath = path.join(DAEMONIUM_CORE_GLOBAL_DIR, file);
  ...
  it("should add and delete a model from config.json", async () => {
    const model: SerializedDaemonium-CoreConfig["models"][number] = {
      title: "Test Model",
      provider: "openai",
      model: "gpt-3.5-turbo",
    };
